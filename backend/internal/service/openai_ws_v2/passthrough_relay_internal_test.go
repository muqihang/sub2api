...
func TestParseUsageAndEnrichCoverage(t *testing.T) {
	...
	parseUsageAndAccumulate(state, []byte(`{"type":"response.completed","response":{"usage":{"input_tokens":2,"output_tokens":1,"cache_creation_input_tokens":9,"input_tokens_details":{"cached_tokens":1,"cache_write_tokens":4},"output_tokens_details":{"image_tokens":3}}}}`), "response.completed", nil)
	...
	require.Equal(t, 4, state.usage.CacheCreationInputTokens, "官方嵌套字段应优先于兼容顶层别名")
