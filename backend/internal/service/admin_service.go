...
func NewAdminService(
	...
	for _, dep := range optionalDeps {
		...
		case AccountRuntimeBlocker:
			runtimeBlocker = typed
		}
	}
...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	peakEnabled, peakStart, peakEnd, peakMultiplier := NormalizePeakRateConfig(
		subscriptionType, input.PeakRateEnabled, input.PeakStart, input.PeakEnd, peakRateMultiplier,
	)
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	group.PeakRateEnabled, group.PeakStart, group.PeakEnd, group.PeakRateMultiplier = NormalizePeakRateConfig(
...
func normalizeOpenAILongContextBillingUpdateExtra(account *Account, extra map[string]any) (map[string]any, error) {
	normalized, err := normalizeOpenAILongContextBillingExtra(account.Platform, extra)
	if err != nil || account.Platform != PlatformOpenAI {
		return normalized, err
	}

	_, provided := extra[openAILongContextBillingEnabledKey]
...
func serviceSensitiveCredentialKeyForAdminMerge(key string) bool {
	return IsSensitiveCredentialKey(strings.TrimSpace(key))
}
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if account.IsCredentialShadow() {
		...
		// 必报错)。须先删影子再改 type。
		shadows, serr := listAccountShadowsByParent(ctx, s.accountRepo, id)
		if serr != nil {
			return nil, serr
		}
		if len(shadows) > 0 {
	...
	if input.Extra != nil {
		...
		normalizedExtra, err := normalizeOpenAILongContextBillingUpdateExtra(account, input.Extra)
		if err != nil {
			return nil, err
		}
...
func (s *adminServiceImpl) DeleteAccount(ctx context.Context, id int64) error {
	...
	shadows, err := listAccountShadowsByParent(ctx, s.accountRepo, id)
	if err != nil {
...
func (s *adminServiceImpl) CreateShadow(ctx context.Context, parentID int64, opts ShadowOptions) (*Account, error) {
	...
	// 2. 一母一影校验
	shadows, err := listAccountShadowsByParent(ctx, s.accountRepo, parentID)
	if err != nil {
	...
	if err := s.accountRepo.Create(ctx, shadow); err != nil {
		if existing, qerr := listAccountShadowsByParent(ctx, s.accountRepo, parentID); qerr == nil && len(existing) > 0 {
			return nil, infraerrors.New(http.StatusConflict, "SPARK_SHADOW_ALREADY_EXISTS",
				"parent account already has a spark shadow account")
		}
		...
		if err := s.accountRepo.BindGroups(ctx, shadow.ID, groupIDs); err != nil {
			if delErr := s.accountRepo.Delete(context.WithoutCancel(ctx), shadow.ID); delErr != nil {
				slog.Error("spark_shadow_bind_groups_rollback_failed",
					"error_class", fmt.Sprintf("%T", delErr))
			}
...
func propagateAccountProxyToShadows(ctx context.Context, repo AccountRepository, parentID int64, proxyID *int64) error {
	shadows, err := listAccountShadowsByParent(ctx, repo, parentID)
	if err != nil {
