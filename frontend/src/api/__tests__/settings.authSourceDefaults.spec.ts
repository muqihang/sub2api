...
describe("admin settings auth source defaults helpers", () => {
  ...
  it("reads nested platform_quotas from settings into auth source state", () => {
    ...
    // 未出现的平台（gemini/antigravity/grok）归一化为 null
  ...
  it("normalizes and sanitizes Grok platform quotas", () => {
    expect(normalizePlatformQuotasMap({
      grok: { daily: 1, weekly: 2, monthly: 3 },
    }).grok).toEqual({ daily: 1, weekly: 2, monthly: 3 });

    expect(sanitizePlatformQuotasMap({
      grok: { daily: 0, weekly: -1, monthly: Number.POSITIVE_INFINITY },
    }).grok).toEqual({ daily: 0, weekly: null, monthly: null });
  });
