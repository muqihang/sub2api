...
Sub2API 可以把已选中的 Anthropic OAuth、Anthropic API key 透传和 Antigravity OAuth 流量转发到 cc-gateway。仅在 cc-gateway 已部署在可信内网时启用 `gateway.cc_gateway`。

启用后，Sub2API 仍负责账号选择和 token 刷新，将已选账号的上游凭证发送给 cc-gateway，并附加内部 `x-cc-*` 控制头：

- `x-cc-gateway-token` 用于 Sub2API 到 cc-gateway 的鉴权。
- `x-cc-provider` 为 `anthropic` 或 `antigravity`。
- `x-cc-token-type` 为 `oauth` 或 `apikey`。
- `x-cc-account-id` 为稳定的 Sub2API 账号 ID。
- `x-cc-egress-bucket` 只传出口桶名称。代理 URL 应配置在 cc-gateway 中，不进入请求头。

账号级出口绑定使用 `account.extra.cc_gateway_egress_bucket`。迁移期会兼容读取 `account.extra.openai_gateway_egress_bucket` 作为 fallback。
