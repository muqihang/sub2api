...
async def make_proxy(tmp_path, status_code: int = 200, response_body: dict | None = None, ws_status: int = 200):
    ...
    upstream_app.router.add_post("/openai/v1/responses", upstream_handler)
    upstream_app.router.add_post("/openai/v1/responses/{tail:.*}", upstream_handler)
    upstream_app.router.add_post("/openai/v1/chat/completions", upstream_handler)
    upstream_app.router.add_get("/openai/v1/models", upstream_handler)
    upstream_app.router.add_get("/v1/models", upstream_handler)
    upstream_app.router.add_get("/openai/v1/responses", upstream_ws)
...
@pytest.mark.asyncio
async def test_only_openai_compatible_paths_are_accepted(tmp_path):
...
@pytest.mark.asyncio
async def test_models_path_maps_to_backend_v1_models(tmp_path):
    upstream, proxy_client, seen, _ = await make_proxy(tmp_path)
    async with proxy_client:
...
async def test_websocket_handshake_injects_managed_authorization(tmp_path):
    ...
    async with proxy_client:
        ...
        assert seen["ws_path"] == "/openai/v1/responses"
