...
const (
	...
	geminiTrieKeyPrefix = "gemini:trie:"
	geminiSessionTTL    = 5 * time.Minute

	openAICacheHealthKeyPrefix = "oa:cache_health:"
	openAICacheHealthTTL       = 6 * time.Hour
)
...
func buildGeminiTrieKey(groupID int64, prefixHash string) string {
	return fmt.Sprintf("%s%d:%s", geminiTrieKeyPrefix, groupID, prefixHash)
}
...
func openAICacheHealthKey(accountID int64, model string) string {
	return fmt.Sprintf("%s%d:%s", openAICacheHealthKeyPrefix, accountID, model)
}

func (c *gatewayCache) RecordOpenAICacheSample(ctx context.Context, accountID int64, model string, inputTokens int64, cacheReadTokens int64) error {
	if accountID <= 0 || model == "" {
		return nil
	}

	key := openAICacheHealthKey(accountID, model)
	nowUnix := time.Now().Unix()

	pipe := c.rdb.Pipeline()
	pipe.HIncrBy(ctx, key, "input", inputTokens)
	pipe.HIncrBy(ctx, key, "cache_read", cacheReadTokens)
	pipe.HIncrBy(ctx, key, "samples", 1)
	pipe.HSet(ctx, key, "updated_unix", nowUnix)
	pipe.Expire(ctx, key, openAICacheHealthTTL)
	_, err := pipe.Exec(ctx)
	return err
}

func (c *gatewayCache) GetOpenAICacheHealthBatch(ctx context.Context, accountIDs []int64, model string) (map[int64]*service.OpenAICacheHealthInfo, error) {
	result := make(map[int64]*service.OpenAICacheHealthInfo, len(accountIDs))
	if len(accountIDs) == 0 || model == "" {
		return result, nil
	}
	...
	cmds := make(map[int64]healthCmd, len(accountIDs))
	pipe := c.rdb.Pipeline()
	for _, accountID := range accountIDs {
		key := openAICacheHealthKey(accountID, model)
		cmds[accountID] = healthCmd{
			cmd: pipe.HMGet(ctx, key, "input", "cache_read", "samples", "updated_unix"),
		}
	}

	_, _ = pipe.Exec(ctx) // key 不存在视为正常情况

	for _, accountID := range accountIDs {
		info := &service.OpenAICacheHealthInfo{}
		raw := cmds[accountID].cmd.Val()
		if len(raw) == 4 {
			info.InputTokens = parseInt64Any(raw[0])
			info.CacheReadTokens = parseInt64Any(raw[1])
			info.Samples = parseInt64Any(raw[2])
			if updatedUnix := parseInt64Any(raw[3]); updatedUnix > 0 {
...
func parseInt64Any(v any) int64 {
	...
	case []byte:
		...
		return n
	default:
		n, _ := strconv.ParseInt(fmt.Sprint(typed), 10, 64)
		return n
	}
}
...
func (c *gatewayCache) FindGeminiSession(ctx context.Context, groupID int64, prefixHash, digestChain string) (uuid string, accountID int64, found bool) {
	...
	trieKey := buildGeminiTrieKey(groupID, prefixHash)
	ttlSeconds := int(geminiSessionTTL.Seconds())
