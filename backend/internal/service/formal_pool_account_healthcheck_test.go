...
func TestFormalPoolGatewayHealthcheckRunnerSafeErrorClassificationMatrix(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "cc gateway control plane error header", status: http.StatusForbidden, headers: seenAndRaw([2]string{"X-CC-Gateway-Error-Code", "formal_pool_context_mismatch"}), body: `{"type":"error"}`, wantCode: "formal_pool_context_mismatch", wantBucket: "cc_gateway"},
