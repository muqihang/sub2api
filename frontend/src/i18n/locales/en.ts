export default {
  ...
  common: {
    ...
    close: 'Close',
      ...
      modelCatalogHint: 'Models available through this connection',
      billing: 'Usage & billing',
      billingHint: 'Unified wallet billing, per-model pricing',
    enabled: 'Enabled',
    disabled: 'Disabled',
	    total: 'Total',
	    balance: 'Balance',
  ...
  keys: {
    ...
    augmentOnlyLabel: 'Augment only',
    augmentOnlyHint: 'Use this key only for Augment traffic and only with Augment-enabled groups.',
    statusLabel: 'Status',
    ...
    augmentGroupRequired: 'Please select an Augment-enabled group',
    usage: 'Usage',
    today: 'Today',
    total: 'Last 30d',
    quota: 'Quota',
    ...
    useKeyModal: {
      ...
      augmentOnly: {
        title: 'Augment-only key',
        description: 'This key is reserved for the dedicated Augment group and Augment Gateway flows. Do not reuse it as a generic Codex, Claude Code, Gemini CLI, or OpenCode key.',
        quickLoginNote: 'Official Quick Login only grants the official cloud session. Users still need this dedicated Augment-only API key when using the Augment Gateway.',
        sharedWalletNote: 'Billing still settles against the shared wallet. Augment attribution stays on the Augment group and Augment client_product.'
      },
      openai: {
    ...
    zhumengAgent: {
      title: 'Codex Quick Connect',
      quickDescription: 'Quickly connect a Codex client using this key. For full management, visit the Codex entry page.',
      quickSetup: 'Quick connect',
      creating: 'Setting up...',
      goToEntryPage: 'Go to Codex Entry Center →',
      keyIdRequired: 'This key is missing an ID and cannot be used for quick connect.',
    },
  ...
  affiliate: {
    ...
    tips: {
      title: 'How It Works',
      growthTitle: 'Invite more, unlock higher rebate tiers',
      line1: 'Share your affiliate code or invite link with new users.',
      line2: 'When invitees recharge, you receive {rate} of the recharge as rebate quota.',
      line3: 'Transfer rebate quota to balance at any time.',
      line4: 'Newly earned rebates may have a waiting period before they can be transferred.',
      tierProgress: '{count} effective paid invitees in the last {days} days; {remaining} more to reach the next tier ({threshold}).',
      inviteeBonus: 'The invitee receives a {rate} bonus on their first real payment.',
      tierTitle: 'Your current rebate tiers',
      effectiveCount: 'You have {count} effective paid invitees in the last {days} days.',
      tierFrom: 'From {count} effective invitees',
      growthNote: 'Invite more to unlock a higher rebate rate on future real payments; total earnings are uncapped.'
    }
  },
  ...
  admin: {
    ...
    augmentGateway: {
      ...
      description: 'Admin surface for the dedicated Augment group, provider execution pools, official session capacity, and shared-wallet billing visibility.',
      operationalGuidance: 'Entitlement groups grant end-user access. Provider routing groups are execution pools only. Users still need dedicated Augment-only API keys, and Official Quick Login does not replace that key requirement.',
      sharedWalletDescription: 'Augment spend still draws from the shared wallet. Attribution remains on the Augment group and Augment client_product so non-Augment business keeps the same wallet model.',
      singleActiveKey: 'Quick Login and billing summary can auto-resolve the gateway key only when the user has exactly one active Augment-only key. If multiple active Augment-only keys exist, operators must resolve the ambiguity first.',
      entitlementGroups: 'Entitlement Groups',
      entitlementGroupsDescription: 'User-facing groups with augment_gateway_entitled enabled. These control who can use Augment Gateway, not which provider pool executes requests.',
      providerRoutingGroups: 'Provider Routing Groups',
      providerRoutingDescription: 'One execution group per provider. These groups back Augment Gateway traffic and are distinct from entitlement groups assigned to users.',
      routePolicyVersion: 'Route Policy',
      configuredRoutePolicyVersion: 'Configured Route Policy',
      executedRoutePolicyVersion: 'Executed Route Policy',
      manageGroups: 'Manage Groups',
      manageAccounts: 'Manage Accounts',
      openQuickLogin: 'Open Quick Login',
      activePoolSessions: 'Active Pool Sessions',
      healthyPoolSessions: 'Healthy Pool Sessions',
      totalPoolSessions: 'Total Pool Sessions',
      cacheHitRatio: 'Cache Hit Ratio',
      estimatedCost: 'Estimated Cost',
      settledCost: 'Settled Cost',
      sharedWalletFreeQuota: 'Shared Wallet Free Quota',
      sharedWalletPaidBalance: 'Shared Wallet Paid Balance',
      cacheReadTokens: 'Cache Read Tokens',
      sourcePriority: 'Source Priority',
      providerGroups: 'Provider Groups',
      models: 'Models',
      modelsDescription: 'Visible models inherit provider execution from the routing groups above. Smoke status must pass, and explicit catalog pricing must exist, before a model can be enabled for admins to expose.',
      enableModel: 'Enable',
      disableModel: 'Disable',
      enabledState: 'Enabled',
      disabledState: 'Disabled',
      visibleState: 'Visible',
      notVisibleState: 'Not Visible',
      explicitPricingState: 'Explicit Pricing Ready',
      missingExplicitPricingState: 'Explicit Pricing Missing',
      visible: 'Visible',
      hidden: 'Hidden',
      officialSessions: 'Official Sessions',
      sessionPoolDescription: 'Captured official sessions are shared execution capacity. Diagnostics remain redacted; use disable or require relogin before revoke when you want to preserve the pool row.',
      ...
      revoke: 'Revoke',
      disable: 'Disable',
      requireRelogin: 'Require Relogin',
      diagnostics: 'Diagnostics',
      usage: 'Usage',
      usageDescription: 'Recent Augment usage rows show request scope, feature scope, entitled group, session correlation, and route policy version without exposing secrets.',
      provider: 'Provider',
      group: 'Group',
      groupId: 'Group ID',
      health: 'Health',
      ...
      status: 'Status',
      accountCoverage: 'Active / Total Accounts',
      model: 'Model',
      scope: 'Scope',
      ...
      emptyEntitlementGroups: 'No active entitlement groups are currently marked for Augment Gateway.',
      emptyUsage: 'No Augment Gateway usage rows found.',
    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          growthModeLegacy: 'Fixed rate (compatibility mode)',
          growthModeTiered: 'Tiered growth (90-day effective invites)',
          growthModeHint: 'Switching does not rewrite historical ledger entries. Tiered mode uses recent effective paid invitees; tiers replace rather than stack.',
          tierWindowDays: 'Effective Invite Window (days)',
          tierWindowDaysHint: 'Only successful real-payment balance or subscription orders with a non-zero net payment count; each invitee counts once.',
          tierRules: 'Tier Rules',
          tierInviteeThreshold: 'Effective invitees ≥',
          tierRate: 'Rebate rate',
          addTier: 'Add tier',
          tierRulesHint: 'The first tier must start at 0. The recommended defaults are 0/3/10/25 and 8%/10%/12%/15%.',
          inviteeBonusRate: 'Invitee First-Payment Bonus',
          inviteeBonusRateHint: 'Granted once on the first real payment recharge; refunds reverse it proportionally.',
          effectivePaymentMin: 'Minimum Net Payment for Eligibility',
          effectivePaymentMinHint: '0 means no extra threshold; the order still needs a positive net payment.',
      ...
      gatewayForwarding: {
        ...
        clientDatelineNormalizationHint: 'Default on. Rewrites the client dateline sentence in Anthropic OAuth/Setup Token requests back to a canonical ASCII apostrophe and hyphen date format. Applies to system prompts and <system-reminder> blocks only; API-Key accounts are unaffected.',
      ...
      betaPolicy: {
        ...
        userIds: 'User IDs',
        userIdsHint: 'Leave empty to apply to all API key owners.',
      ...
      user_error_view: {
        ...
        description: 'When enabled, users can see a redacted view of their failed requests on the usage page (no provider-side details). Requires ops monitoring enabled to have data.',
      },
  ...
  payment: {
	...
	plugin: {
	  augment: {
        ...
        estimatedCost: 'Estimated Cost',
        settledCost: 'Settled Cost',
        sharedWalletTitle: 'Shared Wallet Billing',
        sharedWalletDescription: 'Augment traffic uses the shared wallet for settlement, while usage attribution stays on the dedicated Augment group and Augment client_product. Official Quick Login does not replace the required Augment-only API key.',
        singleActiveKey: 'Automatic gateway-key resolution for the quick-login summary requires exactly one active Augment-only key. If there are multiple active Augment-only keys, resolve that ambiguity first.',
        sharedWalletFreeQuota: 'Shared Wallet Free Quota',
        sharedWalletPaidBalance: 'Shared Wallet Paid Balance',
  ...
  codex: {
    title: 'Codex',
    description: 'Manage Codex client connections',
    loading: 'Loading connection status...',
    retry: 'Reload',
    hero: {
      eyebrow: 'Zhumeng Codex entry center',
      title: 'Connect Codex clients',
      setupDescription: 'Use the models already enabled on your account from the official Codex Desktop / CLI. Finish 3 steps and this machine can start chatting directly.',
      consoleDescription: 'You already have {count} connected devices. This page is for status, adding another device, and handling exceptions.',
      notConnected: 'Not connected',
      awaitingLocal: 'Waiting for local pairing',
      syncing: 'Waiting for catalog sync',
      connected: 'Console ready',
    },
    wizard: {
      steps: {
        credential: 'Create access credential',
        credentialSub: 'First decide whether this connection should use a dedicated credential or reuse an existing key.',
        attach: 'Finish local pairing',
        attachSub: 'Write this connection into the local client without manual config edits.',
        verify: 'Verify model availability',
        verifySub: 'Wait for the first model catalog sync, then move into the console.',
      },
      step1: {
        title: 'Prepare the credential for this connection',
        description: 'The standalone page defaults to a dedicated Codex credential, but you can switch to reusing an existing key. The page explains the product difference without exposing implementation details.',
        labelTitle: 'Credential label',
        labelPlaceholder: 'For example: My MacBook',
        labelHint: 'Only used for your own recognition, such as “My MacBook” or “Office machine”.',
        modeTitle: 'Choose how to connect',
        modeDescription: 'For first-time setup, a dedicated Codex credential is usually the clearest choice.',
        modeIndependent: 'Create a dedicated Codex credential',
        modeIndependentDescription: 'Status, devices, and revocation stay separate from normal API keys. Recommended for the first setup.',
        modeReused: 'Reuse an existing API key',
        modeReusedDescription: 'Skip new creation and connect with an existing key. Status will be shared with that key.',
        startSetup: 'Start connection',
        selectKey: 'Choose a key to reuse',
        selectKeyPlaceholder: 'Select an existing key',
        selectKeyHint: 'Choose an existing key before starting the connection.',
        selectedKeyHint: 'Currently reusing: {name}',
        footerHint: 'If you want to connect another computer later, do it from the console. The page should not pretend this is another first-time onboarding flow.',
        sideWhatTitle: 'What this connection will do',
        sideWhatBody: 'Register a Codex device under your account and sync the currently available models to the local client. Billing still follows the model that is actually used and is deducted from the same wallet balance.',
        sideNoChangeTitle: 'What it will not change',
        sideNoChangeBody: 'It will not rewrite your existing API keys, switch groups for you, or move balances around. Later you can stop usage per device or per credential.',
        sideEnvTitle: 'Required local environment',
        sideEnvBody: 'Supports the official Codex Desktop / CLI. If the client version is too old, the console will show an upgrade direction instead of trapping users in a vague error.',
      },
      step2: {
        title: 'Finish the local connection',
        description: 'This page only hands the connection payload to the local client. You can open the local app directly or copy the CLI path if that is how you work.',
        launchLabel: 'Step 2 · Local connection',
        launchTitle: 'Open the local client to continue',
        launchDescription: 'Use the graphical client first. The CLI path is for people who already use Codex from the terminal.',
        pendingBadge: 'Waiting for local confirmation',
        openLocal: 'Open local client',
        regenerateAndOpen: 'Regenerate and open local client',
        copyCli: 'Copy CLI command',
        cliTitle: 'Command-line connection',
        regenerate: 'Regenerate pairing code',
        diagnose: 'View diagnostics',
        diagnosing: 'Diagnosing...',
        backToCredential: 'Back to connection method',
        helpTitle: 'If the client did not open',
        helpItem1: 'Confirm the official Codex client is installed and signed in locally.',
        helpItem2: 'If the browser blocked the launch, continue with the CLI command instead.',
        helpItem3: 'Diagnostics are read-only and do not change your local client or server state.',
        sessionLabel: 'Current connection label',
        expiresLabel: 'Pairing code expires at',
      },
      step3: {
        title: 'Wait for the first model catalog sync',
        description: 'The client has started connecting. Now the page only waits for the first catalog sync to complete, then it moves into the console automatically.',
        verifying: 'Sync in progress',
        panelTitle: 'Checking whether the models have appeared in the client',
        panelDescription: 'The technical exit condition in this stage is the first catalog sync completing. In other words, the setup session has received first_catalog_synced_at.',
        refresh: 'Refresh status',
        diagnose: 'View diagnostics',
        diagnosing: 'Diagnosing...',
        reopenLocal: 'Reopen client',
        regenerateAndReopen: 'Regenerate and reopen client',
        exitTitle: 'When the page enters the console',
        exitDescription: 'Once this connection record receives its first catalog sync timestamp, the client has the first visible model list and the onboarding state is finished.',
      },
    },
    console: {
      title: 'Codex Console',
      description: 'You currently have {count} connected devices. The page-level primary action is adding a new device; device repair and diagnostics stay split below.',
      addDevice: 'Connect new device',
      bannerLabel: 'Continue from the console',
      newDeviceInProgress: 'New device connection in progress',
      bannerDescription: 'You already have working devices, so the page stays in console mode and only shows the in-progress setup as a banner here.',
      openLocal: 'Open client',
      regenerateAndOpen: 'Regenerate and open client',
      copyCli: 'Copy command',
      stateHealthy: 'Focused device is healthy',
      stateCatalogStale: 'Model catalog needs refresh',
      stateOffline: 'Device is offline',
      stateAttention: 'Device needs attention',
      statePending: 'Waiting for status update',
      devices: 'Connected devices',
      devicesDescription: 'Each device keeps its own status. A device exception should not pull the whole page back into first-time onboarding.',
      noDevices: 'There are no connected devices yet.',
      resync: 'Resync',
      repair: 'Attempt repair',
      diagnose: 'View diagnostics',
      reattach: 'Reconnect this device',
      removeDevice: 'Disconnect device',
      removeDeviceOnly: 'Remove device only',
      upgradeClient: 'View upgrade guide',
      revokeCredential: 'Revoke this access credential',
      disconnectDevice: 'Disconnect this device',
      troubleshoot: 'Troubleshoot',
      troubleshootDescription: 'Only repair and diagnose belong here. They should never be merged into one catch-all action.',
      diagnoseTitle: 'Connection diagnostics',
      diagnoseDescription: 'Below is the structured result returned by the diagnostic endpoint.',
      diagnoseOk: 'All checks passed',
      diagnoseFail: 'Issues found. Follow the hints one by one.',
      close: 'Close',
      modelCatalog: 'Available models',
      modelCatalogHint: 'Shows the Codex gateway model catalog currently available to Codex App.',
      modelCatalogEmpty: 'The model catalog summary is not available yet, but you can still finish connection and status handling first.',
      billing: 'Usage & billing',
      billingHint: 'Shared wallet deduction, priced by model. This explanation should read like product copy, not a technical spec.',
      balanceLabel: 'Current wallet balance',
      billingRuleLabel: 'Billing rule',
      billingRuleBody: 'Charges still come from your current wallet; each model keeps its own price rule.',
      reuseSourceLabel: 'Relation to the access credential',
      reuseSourceBody: 'Whether you use a dedicated credential or reuse an existing key only changes the connection relationship, not the billing hierarchy.',
      topUpCta: 'Top up / purchase',
      pricingPending: 'Model-based pricing',
      pricingBillingMode: 'Billing mode',
      pricingSource: 'Pricing source',
      unitPerRequest: '/ request',
      perRequestPrice: 'Per-request price',
      cacheWritePrice: 'Cache write',
      cacheReadPrice: 'Cache hit',
      pricingIntervals: 'Tiered pricing',
      pricingSourceLabels: {
        channel: 'Channel pricing',
        litellm: 'Model pricing catalog',
        fallback: 'System fallback',
      },
      deviceHealthyTitle: 'This device is healthy',
      deviceHealthyBody: 'Heartbeat is normal and the model catalog has synced successfully. You can resync again or inspect diagnostics.',
      deviceCatalogStaleTitle: 'The model catalog needs another sync',
      deviceCatalogStaleBody: 'The device is online, but the latest model catalog has not refreshed in time. The primary action here is resync, not repair.',
      deviceOfflineTitle: 'This device is currently offline',
      deviceOfflineBody: 'Try repair first. If it still does not come back, inspect diagnostics next. Repair and diagnose always stay separate.',
      deviceCredentialRevokedTitle: 'The access credential is no longer valid',
      deviceCredentialRevokedBody: 'This is not just an offline device. The connection relationship itself has been invalidated, so the primary action must be reconnect.',
      deviceClientOutdatedTitle: 'The client version is too old',
      deviceClientOutdatedBody: 'Upgrade the local client first, then return to check the status again.',
      deviceStateLabels: {
        healthy: 'Healthy',
