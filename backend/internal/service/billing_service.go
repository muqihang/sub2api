...
type ModelPricing struct {
	...
	CacheCreationPriceExplicit         bool    // 是否由渠道定价显式设定（为 true 时即使 == 0 也不回退）
...
type BillingService struct {
	...
	fallbackWarns  sync.Map                 // normalized model -> struct{}, process-local log dedupe
}
...
func (s *BillingService) initFallbackPricing() {
	...
	s.fallbackPrices["claude-fable-5"] = &ModelPricing{
		...
		CacheCreationPricePerToken: 12.5e-6, // $12.5 per MTok
...
func (s *BillingService) getFallbackPricing(model string) *ModelPricing {
	...
	// 按模型系列匹配
	if strings.Contains(modelLower, "claude-fable-5") || strings.Contains(modelLower, "claude-fable5") {
...
func (s *BillingService) GetModelPricing(model string) (*ModelPricing, error) {
	...
	if fallback != nil {
		if _, loaded := s.fallbackWarns.LoadOrStore(model, struct{}{}); !loaded {
