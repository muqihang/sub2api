...
func newOpenAIChatFallbackFailureEvent(account *Account, resp *http.Response, message string) OpsUpstreamErrorEvent {
...
func (s *OpenAIGatewayService) forwardAnthropicViaRawChatCompletions(
	...
	reasoningEffort = ApplyChineseThinkingEffortFallback(reasoningEffort, body, billingModel)
	...
	if resp.StatusCode >= 400 {
		...
		if s.shouldFailoverOpenAIUpstreamResponse(resp.StatusCode, upstreamMsg, respBody) {
			appendOpsUpstreamError(c, newOpenAIChatFallbackFailureEvent(account, resp, upstreamMsg))
