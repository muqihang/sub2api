...
//
// schema 中没有 success bool 列；新增列要做迁移，风险大；这里用"正成本或实际用量证据"
// 作为代理。actual_cost > 0 覆盖常规付费请求，token/image 字段覆盖 AGNES 等零成本成功请求，
// 仍会排除所有 tokens=0、image=0、cost=0 的 failed-request placeholder。
// 配合 `FROM usage_logs ul` JOIN 查询使用。
const usageLogSuccessFilterUL = "(ul.actual_cost > 0 OR COALESCE(ul.input_tokens, 0) > 0 OR COALESCE(ul.output_tokens, 0) > 0 OR COALESCE(ul.cache_creation_tokens, 0) > 0 OR COALESCE(ul.cache_read_tokens, 0) > 0 OR COALESCE(ul.image_output_tokens, 0) > 0 OR COALESCE(ul.image_count, 0) > 0)"
...
func appendUsageLogBillingModeQueryFilter(query string, args []any, billingMode string) (string, []any) {
	conditions, nextArgs := appendUsageLogBillingModeWhereCondition(nil, args, billingMode)
	if len(conditions) == 0 {
...
func appendUsageLogEntityWhereConditions(conditions []string, args []any, filters UsageLogFilters) ([]string, []any) {
	return appendUsageLogEntityWhereConditionsWithPrefix(conditions, args, "", filters)
}

func appendUsageLogEntityWhereConditionsWithPrefix(conditions []string, args []any, prefix string, filters UsageLogFilters) ([]string, []any) {
	...
	if filters.EntityID > 0 {
		conditions = append(conditions, fmt.Sprintf("%s = $%d", column("entity_id"), len(args)+1))
		args = append(args, filters.EntityID)
	}
	if entityType := strings.TrimSpace(filters.EntityType); entityType != "" {
		conditions = append(conditions, fmt.Sprintf("%s = $%d", column("entity_type"), len(args)+1))
		args = append(args, entityType)
	}
	if claimed := strings.TrimSpace(filters.ClaimedEntityID); claimed != "" {
		conditions = append(conditions, fmt.Sprintf("%s = $%d", column("claimed_entity_id"), len(args)+1))
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	entityID := nullInt64(log.EntityID)
	entityType := nullString(log.EntityType)
	claimedEntityID := nullString(log.ClaimedEntityID)
...
func (r *usageLogRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters UsageLogFilters) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	conditions, args = appendUsageLogEntityWhereConditions(conditions, args, filters)
...
func shouldUseFastUsageLogTotal(filters UsageLogFilters) bool {
	...
	return filters.UserID == 0 &&
		...
		filters.AccountID == 0 &&
		filters.EntityID == 0 &&
		strings.TrimSpace(filters.EntityType) == "" &&
		strings.TrimSpace(filters.ClaimedEntityID) == ""
}
...
func (r *usageLogRepository) GetGroupStatsWithUsageFilters(ctx context.Context, startTime, endTime time.Time, filters UsageLogFilters) (results []usagestats.GroupStat, err error) {
	...
	conditions := make([]string, 0, 3)
	conditions, args = appendUsageLogEntityWhereConditionsWithPrefix(conditions, args, "ul", filters)
	for _, condition := range conditions {
		query += " AND " + condition
	}
	query, args = appendRequestTypeOrStreamQueryFilter(query, args, filters.RequestType, filters.Stream)
...
func (r *usageLogRepository) GetStatsWithFilters(ctx context.Context, filters UsageLogFilters) (*UsageStats, error) {
	...
	runEndpoints := func(c context.Context) {
		res, err := r.getEndpointStatsByColumnWithFilters(c, "inbound_endpoint", start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode)
		if err != nil {
	...
	runUpstream := func(c context.Context) {
		res, err := r.getEndpointStatsByColumnWithFilters(c, "upstream_endpoint", start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode)
		if err != nil {
			if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
	...
	runPaths := func(c context.Context) {
		res, err := r.getEndpointPathStatsWithFilters(c, start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode)
		if err != nil {
			if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
