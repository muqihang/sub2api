export default {
  ...
  admin: {
    ...
    augmentGateway: {
      ...
      description: '用于区分 Augment 授权分组、Provider 执行分组、官方 Session 池和计费使用情况的管理页面。',
      operationalGuidance: '授权分组决定终端用户是否能使用 Augment Gateway；Provider 执行分组只负责实际转发；官方 Session 诊断必须保持脱敏，并通过采集、停用、要求重登、撤销等动作维护。',
      ...
      models: '模型',
      modelsDescription: '模型是否可见依赖上方的 Provider 执行分组。只有 smoke 状态通过后，模型才允许在管理端启用。',
      enableModel: '启用',
      disableModel: '停用',
      enabledState: '已启用',
      disabledState: '已停用',
