...
func (h *ConcurrencyHelper) acquireUserSlotWithWaitTimeout(c *gin.Context, userID int64, maxConcurrency int, timeout time.Duration, isStream bool, streamStarted *bool) (func(), error) {
	...
	// Try to acquire immediately. Immediate acquisitions never enter the wait queue.
	releaseFunc, acquired, err := h.TryAcquireUserSlot(ctx, userID, maxConcurrency)
	if err != nil {
		return nil, err
	}
	...
	// Need to wait - handle streaming ping if needed. Skip waitForSlot's immediate try
	// because we already attempted once before entering the wait queue.
