  ...
  >
    ...
    <div class="flex flex-1 flex-col p-4">
      ...
      <div class="mb-3 grid grid-cols-2 gap-x-3 gap-y-1 rounded-lg bg-gray-50 px-3 py-2 text-xs dark:bg-dark-700/50">
        ...
        <div v-if="peakRateDisplay" class="col-span-2 flex items-center justify-between">
          <span class="text-gray-400 dark:text-dark-500">{{ t('payment.planCard.peakRate') }}</span>
          <span class="font-medium text-amber-700 dark:text-amber-300">{{ peakRateDisplay }}</span>
        </div>
...
const peakRateDisplay = computed(() => {
  const window = formatPeakRateWindow(props.plan, serverTimezoneLabel(appStore.cachedPublicSettings?.server_utc_offset))
  if (!window) return ''
  return `${window} ${peakRateMultiplierLabel(props.plan.peak_rate_multiplier)}`
})
