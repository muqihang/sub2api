...
func mergeOpenAIBetaValues(values ...string) string {
...
// IsOpenAIWSIngressAccountRetryable reports whether ingress websocket proxy
// should retry with another account for the current client connection.
// Retry is only safe when no downstream business event has been written yet.
func newOpenAIWSIngressAccountFailoverError(reason string, err error) *UpstreamFailoverError {
	message := "openai websocket upstream unavailable"
	if trimmed := strings.TrimSpace(reason); trimmed != "" {
		message = "openai websocket " + trimmed + " failed"
	}
...
func IsOpenAIWSIngressAccountRetryable(err error) bool {
	if err == nil {
		return false
	}
	...
	if errors.As(err, &fallbackErr) && fallbackErr != nil {
		...
		switch reason {
		case "dial_failed", "acquire_timeout", "acquire_conn", "conn_queue_full", "upstream_5xx":
			return true
		default:
			return false
		}
...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	parseClientPayload := func(raw []byte) (openAIWSClientPayload, error) {
		...
		if account != nil && account.Type == AccountTypeOAuth {
			...
			applyCodexOAuthTransform(payload, isCodexCLI, false)
			if turnMetadata != "" {
				setOpenAIWSTurnMetadata(payload, turnMetadata)
			}
			rebuilt, marshalErr := json.Marshal(payload)
		...
		if isCodexCLI && codexImageGenerationExplicitToolPolicy == codexImageGenerationExplicitToolPolicyStrip {
			if stripped, changed, stripErr := stripOpenAIImageGenerationToolFromRawPayload(normalized); stripErr != nil {
				...
				normalized = stripped
				logOpenAIWSModeInfo("ingress_ws_codex_image_tool_stripped_by_policy")
			}
		}
	...
	refreshIngressRouteState := func(payload openAIWSClientPayload) {
		...
		if stateStore != nil && payload.previousResponseID != "" {
			scopedPreviousResponseID := EntityScopedStateKeyFromContext(requestContext(c), payload.previousResponseID)
			if connID, ok := stateStore.GetResponseConn(scopedPreviousResponseID); ok {
				preferredConnID = connID
			}
		}
	...
	if forceHTTPBridge || s.shouldBridgeOpenAIWSHTTP(account, firstPayload.payloadBytes, firstPayload.previousResponseID) {
		...
		// Keep the first turn as the stable Grok cache seed so follow-up turns
		// retain the same upstream prompt-cache identity.
		...
		for {
			...
			if isTerminalEvent {
				...
				result := &OpenAIForwardResult{
					...
					ReasoningEffort: ApplyChineseThinkingEffortFallback(extractOpenAIReasoningEffortFromBody(payload, mappedModel, originalModel), payload, mappedModel),
					Stream:          reqStream,
					OpenAIWSMode:    true,
					ResponseHeaders: lease.HandshakeHeaders(),
	...
	for {
		...
		if sessionLease == nil {
			...
			if acquireErr != nil {
				err := fmt.Errorf("acquire upstream websocket: %w", acquireErr)
				if IsOpenAIWSIngressAccountRetryable(err) {
					return newOpenAIWSIngressAccountFailoverError("acquire", err)
				}
				return err
			}
		...
		if shouldPreflightPing {
			if pingErr := sessionLease.PingWithTimeout(openAIWSConnHealthCheckTO); pingErr != nil {
				...
				acquiredLease, acquireErr := acquireTurnLease(turn, preferredConnID, forcePreferredConn)
				if acquireErr != nil {
					err := fmt.Errorf("acquire upstream websocket after preflight ping fail: %w", acquireErr)
					if IsOpenAIWSIngressAccountRetryable(err) {
						return newOpenAIWSIngressAccountFailoverError("acquire", err)
					}
					return err
		...
		if relayErr != nil {
			...
			sessionLease.MarkBroken()
			if IsOpenAIWSIngressAccountRetryable(relayErr) {
				return newOpenAIWSIngressAccountFailoverError("relay", relayErr)
			}
			return relayErr
		}
		turnRetry = 0
...
func (s *OpenAIGatewayService) selectAccountByPreviousResponseIDForCapability(
	...
	scopedResponseID := EntityScopedStateKeyFromContext(ctx, responseID)
	lookupResponseID := scopedResponseID
	accountID, err := store.GetResponseAccount(ctx, derefGroupID(groupID), lookupResponseID)
	if err != nil || accountID <= 0 {
		// Legacy bindings created before request-entity scoping used the raw response_id.
		// Keep them readable so in-flight chains survive upgrades, while all refreshes below
		// continue writing the scoped key.
		lookupResponseID = responseID
		accountID, err = store.GetResponseAccount(ctx, derefGroupID(groupID), lookupResponseID)
	}
	if err != nil || accountID <= 0 {
		return nil, nil
	...
	// Model/capability mismatches are request-shape specific rather than account-health
	// failures. Preserve the sticky binding so a later compatible turn can resume.
